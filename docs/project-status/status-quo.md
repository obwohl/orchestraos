...
*   **Core Dialect (`OrchestraIR`):** The core dialect is implemented and functional.
    *   **MLIR Properties:** The `target_arch` attribute of `orchestra.task` has been refactored to use the MLIR `Properties` system. This improves performance and type safety.
    *   **`CommitOp` Refactoring:** The `orchestra.commit` operation has been redefined to represent the explicit commitment of a value to a specific memory space. It now takes a single `MemRef` operand and produces a single `MemRef` result. This change aligns the operation with the dialect's focus on scheduling and data movement.
*   **Speculative Execution:** The `--divergence-to-speculation` pass has been temporarily disabled due to a refactoring of the `orchestra.commit` op.
...
*   **Standard Lowering:** The `--lower-orchestra-to-standard` pass has been updated to remove the lowering for `orchestra.commit`.
