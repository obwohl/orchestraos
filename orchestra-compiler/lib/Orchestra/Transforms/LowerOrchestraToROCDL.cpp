...
// This is the original logic for lowering to ROCDL, extracted into its own
// pass.
...
public:
  ...
  void runOnOperation() override {
    ...
    for (auto op : transferOps) {
      ...
      // Get the base addresses of the source and destination memrefs as
      // integers.
      auto sourceIndex =
          builder.create<memref::ExtractAlignedPointerAsIndexOp>(loc, source);
      auto destIndex = builder.create<memref::ExtractAlignedPointerAsIndexOp>(
          loc, dest.getResult());

      // Cast the index type to a standard integer type (i64) for IntToPtrOp.
      auto i64Type = builder.getI64Type();
      auto sourceI64 = builder.create<arith::IndexCastOp>(
          loc, i64Type, sourceIndex.getResult());
      auto destI64 = builder.create<arith::IndexCastOp>(
          loc, i64Type, destIndex.getResult());

      auto sourcePtr = builder.create<LLVM::IntToPtrOp>(
          loc, llvmPointerType, sourceI64.getResult());
      auto destPtr = builder.create<LLVM::IntToPtrOp>(
          loc, llvmPointerType, destI64.getResult());
      ...
      auto globalPtr =
          builder.create<LLVM::AddrSpaceCastOp>(loc, globalPtrType, sourcePtr);
      auto ldsPtr =
          builder.create<LLVM::AddrSpaceCastOp>(loc, ldsPtrType, destPtr);
