...
// Forward declarations for C++ patterns
namespace {
struct FoldConstantCommit;
struct EraseEmptySchedule;
} // namespace
...
void OrchestraDialect::registerCanonicalizationPatterns(
    ::mlir::RewritePatternSet &patterns) {
  // Add DRR patterns
  populateWithGenerated(patterns);
  // Add C++ patterns
  patterns.add<FoldConstantCommit, EraseEmptySchedule>(getContext());
}
...
struct FoldConstantCommit : public mlir::OpRewritePattern<CommitOp> {
  ...
  matchAndRewrite(CommitOp op,
    ...
    auto value = op.getCondition().getDefiningOp<mlir::arith::ConstantOp>().getValue().dyn_cast<mlir::BoolAttr>();
    if (!value) {
      return mlir::failure();
    }
...
mlir::LogicalResult TaskOp::verify() {
  ...
  auto terminator = getBody().front().get Terminator();
  if (!terminator) {
