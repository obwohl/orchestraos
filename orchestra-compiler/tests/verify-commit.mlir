...
func.func @test_valid_single_value(%cond: i1, %true: f32, %false: f32) {
  %0 = orchestra.commit %cond true(%true) false(%false) : (i1, f32, f32) -> f32
  return
}
...
func.func @test_valid_multiple_values(%cond: i1, %t1: f32, %t2: tensor<4xf32>, %f1: f32, %f2: tensor<4xf32>) {
  %0, %1 = orchestra.commit %cond true(%t1, %t2) false(%f1, %f2) : (i1, f32, tensor<4xf32>, f32, tensor<4xf32>) -> (f32, tensor<4xf32>)
  return
}
...
func.func @test_valid_zero_values(%cond: i1) {
  orchestra.commit %cond true() false() : (i1) -> ()
  return
}
...
func.func @test_invalid_mismatched_true_false_count(%cond: i1, %t1: f32, %f1: f32, %f2: f32) {
  // expected-error@+1 {{'orchestra.commit' op has mismatched number of true and false operands}}
...
func.func @test_invalid_mismatched_true_false_types(%cond: i1, %t1: f32, %f1: i32) {
  ...
  %0 = orchestra.commit %cond true(%t1) false(%f1) : (i1, f32, i32) -> f32
  return
}
...
func.func @test_invalid_mismatched_result_count(%cond: i1, %t1: f32, %f1: f32) {
  ...
  %0, %1 = orchestra.commit %cond true(%t1) false(%f1) : (i1, f32, f32) -> (f32, f32)
  return
}
...
func.func @test_invalid_mismatched_result_types(%cond: i1, %t1: f32, %f1: f32) {
  ...
  %0 = orchestra.commit %cond true(%t1) false(%f1) : (i1, f32, f32) -> i32
  return
}
